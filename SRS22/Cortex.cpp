...
namespace SRS22 {
	...
	void Cortex::ComputeNextStateSingleNeuron(const size_t i, CortexThreadStats& threadStats)
	{
		...
		for (int k = 0; k < NEURON_UPSTREAM_LINKS; k++) {
			...
			L.activity = std::clamp(L.activity * settings.linkActivityDecayRate.F() +
				stimulus * settings.linkStimulusToActivityFactor.F(), 0.0f, 1.0f);
		}

		// Now clamp the result.
		neurons.setNext(i, std::clamp(neurons.getNext(i) * settings.chargeDepletionRate.F(), 0.0f, 1.0f));
		checkNanByIdx(i);
	}
	...
	void Cortex::DoLearningSingleNeuron(const size_t i, CortexThreadStats& threadStats) {
		...
		neurons.energyCeiling[i] = std::clamp(neurons.energyCeiling[i] 
			// Deplete energy
			- C * settings.energyDepletionOnFire.F()
			// Recover energy
			+ settings.energyRechargeRate.F(), 0.0f, settings.maxEnergy.F());
		...
		const float learningRateDueToGoodness = 1.0f + brain.overallGoodness * settings.learningRateGoodnessFactor.F();
		...
		for (int k = 0; k < NEURON_UPSTREAM_LINKS; k++) {
			...
			const float learningRateDueToNewnessOfL = 1.0f + ageLogInverse * settings.learningRateAgeFactor.F();
			// Active link learn a little faster.
			const float learningRateDueToActivityOfL = 1.0f * L.activity * settings.linkActivityLearningFactor.F();
			// How fast to forget. 1 is no forgetting, 0.5 is ultra rapid forgetting. Values lik 0.99999 are good.
			const float forgetFactor = settings.learningRateForgetFactor.F() * (1.0f - (1.0 / ((double)ageLog + settings.learningRateForgetLogOffset.I())));
			
			// We are trying to train to predict the future. So use other's charge N ticks ago.
			const float otherCPast = neurons.getPast(L.otherIdx, NEURON_HISTORY - settings.learningRateTicksOffset.I());
			const float otherPastInfluence = CDelta * otherCPast;
			L.weight = std::clamp(L.weight + -otherPastInfluence *
				settings.overallLearnRate.F() *
			...
			L.confidence = L.confidence + 
				otherPastInfluence * 
				L.weight * 
				settings.confidenceForgetFactor.F() *(1.0f - (1.0 / ((double)ageLog + settings.confidenceForgetLogOffset.F())));
		}
	}
